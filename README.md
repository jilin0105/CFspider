...
| **CloakBrowser 反检测** | C++ 源码级 Chromium 补丁，`stealth=True` 或 `browser=True` 一参启用，绕过人机验证 |
...
> 快速、安全、每月免费 5GB 起，专为日常翻墙优化。
> 或手动输入邀请码 **`aVDxxRS0`**。
>
> 如需购买高质量代理 IP（住宅 IP、移动 IP、指定地区），推荐：
> - **Cliproxy**: [https://dash.cliproxy.com/](https://dash.cliproxy.com/) - 支持 HTTP/SOCKS5，覆盖全球多个地区
> - **1024proxy**: [https://1024proxy.com/](https://1024proxy.com/) - 全球住宅 IP，0.49/GB 起
> - **Novproxy**: [https://novproxy.com/](https://novproxy.com/) - 1亿+ 住宅 IP，190 个国家，$0.5/GB 起
...
| 特性 | 说明 |
|------|------|
| **CloakBrowser 深度集成** | 放弃手工 headers 注入，全面迁移到 C++ 源码级 Chromium 补丁反检测 |
| **`stealth=True`** | CloakBrowser `context.request` HTTP 请求：真实 TLS 指纹、真实 UA，无 JS 渲染 |
| **`browser=True`** | CloakBrowser `page.goto()` 完整渲染：执行 JS、触发动态内容、可绕过 Turnstile/CAPTCHA |
| **真实 Chrome 版本号** | 随机从 `124.0.6367.207`–`135.0.7049.52` 池中选取，消除 `0.0.0` 假版本 |
| **异步支持** | `aget/apost` 等所有异步方法同步支持 `stealth`/`browser` 参数（线程池运行 Playwright） |

## v1.9.1 新特性

| 特性 | 说明 |
|------|------|
| **WorkersManager 完整支持** | `cf_proxies` 直接传入 `WorkersManager` 对象，自动提取 `url`、`uuid`、`mode`，无需手动传参 |
| **模式自动识别** | 传入 WorkersManager 时直接使用 `workers.mode`，跳过网络检测，请求更快 |
...
| `cfspider.StealthSession` | 可选 | 支持 | CloakBrowser 会话复用，保持 Cookie/Referer/TLS 指纹 |
...
### 搜索引擎收录 - Bing 搜索"cfspider 爬虫库"

![Bing 搜索 cfspider](bing_result.png)

### 搜索引擎收录 - 百度搜索"Python 爬虫"

![百度搜索 Python 爬虫](baidu_result.png)
...
- **CloakBrowser 深度集成**：`stealth=True`（HTTP 隐身）/ `browser=True`（完整 JS 渲染）/ `StealthSession`（会话复用）
- **真实 Chrome 版本号**：随机从 `124.0.6367.207`–`135.0.7049.52` 池中选取，无 `0.0.0` 假版本
- **异步反检测**：`aget/apost` 等全部异步方法支持 `stealth`/`browser` 参数（线程池运行 Playwright）
...
| 随机延迟 | OK | 请求间随机等待 |
| stealth=True | OK | CloakBrowser context.request ，真实 TLS 指纹 |
| browser=True | OK | CloakBrowser page.goto() JS 渲染，绕过 bot 检测 |
| 真实 Chrome 版本号 | OK | 随机 124-135 的真实 build 号，消除 0.0.0 |
| 异步 stealth/browser | OK | run_in_executor 封装，不阻塞事件循环 |
...
如需使用 `browser=True` / `stealth=True` CloakBrowser 反检测模式，需要额外安装：

```bash
# 安装带浏览器支持的 cfspider（包含 Playwright）
pip install cfspider[browser]

# 安装 CloakBrowser（推荐，源码级 Chromium 补丁，反检测能力远强于 Playwright 原版）
pip install cloakbrowser

# 安装 CloakBrowser 内置 Chromium
python -m cloakbrowser install
```

> **提示**：未安装 CloakBrowser 时自动回退到标准 Playwright Chromium，仍可使用 `browser=True`，但反检测效果弱。
...
### CloakBrowser 反检测（v1.9.4 核心功能）

```python
import cfspider

# ── stealth=True：HTTP 级隐身（真实 TLS 指纹 + 真实 Chrome UA，无 JS 渲染） ──
# 适合：API 接口、无 JS 防护的页面、高频请求
response = cfspider.get("https://httpbin.org/headers", stealth=True)
print(response.json()["headers"]["User-Agent"])
# Mozilla/5.0 ... Chrome/131.0.6778.85 Safari/537.36  ← 真实版本号

# ── browser=True：完整浏览器渲染（执行 JS、绕过 Turnstile/CAPTCHA） ──
# 适合：Cloudflare 5s 盾、动态加载页面、滑块验证
response = cfspider.get("https://example.com", browser=True)
print(response.text)            # 完整 HTML（JS 已执行）
print(response.status_code)     # 200

# ── browser=True + headless=False：有头模式，可目视调试 ──
response = cfspider.get("https://bot.sannysoft.com", browser=True, headless=False)

# ── browser=True + js_eval：页面加载后执行 JS，提取动态数据 ──
response = cfspider.get(
    "https://example.com",
    browser=True,
    js_eval="document.title"
)
print(response.js_result)       # 'Example Domain'

# ── browser=True + screenshot：截图存档 ──
response = cfspider.get(
    "https://example.com",
    browser=True,
...
# ── StealthSession：会话复用，共享 CloakBrowser 实例，自动传递 Cookie / Referer ──
with cfspider.StealthSession(delay=(0.5, 1.5)) as sess:
    r1 = sess.get("https://example.com/page1")
    r2 = sess.get("https://example.com/page2")  # 自动携带上次 Cookie + Referer
    print(f"请求次数: {sess.request_count}")

# ── 异步支持：stealth/browser 在 async/await 中同样可用 ──
import asyncio
...
# v1.9.1+：WorkersManager 完整支持，自动提取 URL、UUID 和模式，无需手动传 uuid
response = cfspider.get(
    "https://httpbin.org/ip",
    cf_proxies=workers  # 自动使用 workers.uuid 和 workers.mode
)
print(response.json())
...
| `uuid` | str | None | VLESS UUID（可选，不填自动获取） |
| `static_ip` | bool | False | 固定 IP 模式（False = 每次换 IP） |
| **`stealth`** | **bool** | **False** | **CloakBrowser HTTP 隐身：真实 TLS 指纹 + 真实 Chrome UA，无 JS 渲染** |
| **`browser`** | **bool** | **False** | **CloakBrowser 完整渲染：执行 JS，可绕过 Turnstile/CAPTCHA** |
| `headless` | bool | True | 浏览器无头模式（`browser=True` 时生效，False = 有头可目视） |
| `humanize` | bool | True | 人类行为模拟（CloakBrowser humanize 模式，鼠标移动等） |
| `wait_until` | str | `'load'` | 页面等待条件：`load` / `domcontentloaded` / `networkidle` |
| `screenshot` | str | None | 截图保存路径（`browser=True` 时可用） |
| `js_eval` | str | None | 页面加载后执行的 JS，结果存入 `response.js_result` |
| `impersonate` | str | None | TLS 指纹模拟（如 `chrome131`），与 `stealth` 互斥 |
| `http2` | bool | False | 启用 HTTP/2（与 `impersonate` 互斥） |
| `delay` | tuple | None | 请求前随机延迟，如 `(1, 3)` 秒 |
...
| `proxies` | dict | None | 普通 HTTP/SOCKS5 代理（与 requests 兼容） |
| `two_proxy` | str | None | 双层代理（格式：`host:port:user:pass`） |

**CloakBrowser 模式速查：**

| 场景 | 推荐参数 |
|------|---------|
| API 接口、无 JS 防护、高频 | `stealth=True` |
| Cloudflare 5s 盾、动态页面 | `browser=True` |
| 有头模式调试 | `browser=True, headless=False` |
| 截图 + 等待 AJAX | `browser=True, wait_until='networkidle', screenshot='out.png'` |
| 提取 JS 动态数据 | `browser=True, js_eval='document.querySelector("#price").innerText'` |
| 会话保持 + 自动 Cookie | `StealthSession()` |
...
## 异步 API（httpx + CloakBrowser）

CFspider 提供基于 httpx 的异步 API，支持 async/await 语法，适合高并发场景。  
**v1.9.4 起**，`aget/apost` 等所有异步方法同步支持 `stealth=True` 和 `browser=True` 参数（通过 `run_in_executor` 在线程池中运行 CloakBrowser，不阻塞事件循环）。
...
async def main():
    ...
    # CloakBrowser 隐身（stealth=True，HTTP 级，无 JS 渲染）
    response = await cfspider.aget("https://httpbin.org/headers", stealth=True)
    print(response.json()["headers"]["User-Agent"])

    # CloakBrowser 完整渲染（browser=True，执行 JS，绕过 CAPTCHA）
    response = await cfspider.aget("https://example.com", browser=True)
    print(response.status_code)

    # browser=True + js_eval
...
| `cfspider.aoptions(url, **kwargs)` | 异步 OPTIONS |
| `cfspider.apatch(url, **kwargs)` | 异步 PATCH |
| `cfspider.astream(method, url, **kwargs)` | 流式请求（上下文管理器） |
| `cfspider.AsyncSession(**kwargs)` | 异步会话（连接池） |

所有异步方法均接受 `stealth`、`browser`、`headless`、`humanize`、`wait_until`、`screenshot`、`js_eval`、`uuid`、`two_proxy` 参数，行为与同步版本完全一致。

## 反检测模式（CloakBrowser 深度集成）

> **v1.9.4 重大升级**：放弃手工 headers 注入，全面使用 **CloakBrowser**（C++ 源码级 Chromium 补丁）作为反检测引擎。真实浏览器指纹、随机真实 Chrome 版本号，彻底解决人机验证问题。

### stealth=True — HTTP 级隐身

通过 CloakBrowser `context.request` 发起请求：无 JS 渲染，但具备真实浏览器 TLS 指纹和真实 Chrome UA。

```python
import cfspider

# 适合：API 接口、无 JS 盾的页面、高频采集（速度快）
response = cfspider.get("https://httpbin.org/headers", stealth=True)
print(response.json()["headers"]["User-Agent"])
# Mozilla/5.0 ... Chrome/131.0.6778.85 Safari/537.36

# 带延迟
response = cfspider.get("https://example.com", stealth=True, delay=(0.5, 2))

# 配合 Workers 代理（隐身 + 动态 IP）
response = cfspider.get(
    "https://example.com",
    cf_proxies="https://your-workers.dev",
    stealth=True
...
response = cfspider.get(
    "https://example.com",
    browser=True,
    js_eval='document.querySelector("h1").innerText'
)
print(response.js_result)   # 'Example Domain'

# 配合 Workers 代理（完整渲染 + 动态 IP）
response = cfspider.get(
    "https://example.com",
    cf_proxies="https://your-workers.dev",
    browser=True
...
`StealthSession` 复用同一个 CloakBrowser 实例：保持 Cookie、自动 Referer、共享 TLS 指纹。
...
with cfspider.StealthSession(
    ...
    auto_referer=True       # 自动添加 Referer（默认开启）
) as session:
    r1 = session.get("https://example.com/page1")
    r2 = session.get("https://example.com/page2")  # 自动带 Cookie + Referer
    r3 = session.post("https://example.com/api", json={"key": "value"})

    print(f"请求次数: {session.request_count}")
    print(f"Cookie: {session.get_cookies()}")
